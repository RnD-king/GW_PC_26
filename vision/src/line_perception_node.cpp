...
std::string RuntimeConfigPath() {
  if (const char *env_path = std::getenv("YOLO26_RUNTIME_CONFIG");
...
std::string ReadRuntimeString(const std::string &param_name) {
  try {
    ...
    std::cerr << "[WARN] Failed to load YOLO26 runtime config: " << e.what()
              << std::endl;
  }
  return {};
}
...
std::string DefaultYolo26EnginePath() {
  if (const char *env_path = std::getenv("YOLO26_ENGINE_PATH");
      env_path != nullptr && env_path[0] != '\0') {
    return std::string(env_path);
  }
...
public:
  LinePerceptionNode() : rclcpp::Node("line_perception_node") {
    ...
    declare_parameter<std::string>("engine_path", DefaultYolo26EnginePath());
    declare_parameter<int>("line_class_id", 0);
    declare_parameter<double>("conf_thres", 0.60);
    ...
    yolo_ =
        std::make_unique<YoloTrtEngine>(engine_path_, 640, 640, conf_thres_);
    point_extractor_ =
